...
static NSString *const kReceiverAppID = @"1B082765";  //Replace with your app id
...
- (void)connectToDevice:(GCKDevice *)device {
	...
	if ([self.delegate respondsToSelector:@selector(doConnecting:)]) {
		[self.delegate doConnecting:self.selectedDevice];
	}
...
- (void)setPlaybackTime:(double)newTime {
	if (_streamDuration > 0 && self.isConnected) {
		[self.mediaControlChannel seekToTimeInterval:newTime];
	}
}
...
didFailToConnectToApplicationWithError:(NSError *)error {
	...
	[self showError:NSLocalizedString(@"failed_to_connect_to_chromecast", nil)];
...
- (void)deviceManager:(GCKDeviceManager *)deviceManager didDisconnectWithError:(GCKError *)error {
	...
	if (error != nil) {
		//[self showError:error];
		[self showError:NSLocalizedString(@"lost_chromecast_connection", nil)];
	}
...
- (BOOL)loadMedia:(NSURL *)url
	...
	if (thumbnailURL) {
		if (music)
			[metadata addImage:[[GCKImage alloc] initWithURL:thumbnailURL width:320 height:320]];
		else
			[metadata addImage:[[GCKImage alloc] initWithURL:thumbnailURL width:320 height:180]];
	}
...
- (void)initControls {
	...
	_chromecastBarButton.tintColor = nil;
	
	// Create toolbar buttons for the mini player.
//	CGRect frame = CGRectMake(0, 0, 49, 37);
//	_toolbarThumbnailImage =
//	[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"video_thumb_mini"]];
